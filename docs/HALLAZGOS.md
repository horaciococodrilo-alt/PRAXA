...
| `H-M04.1-02` | Pendiente | M06.3a (reasignado desde M06.2a por decisión del usuario, 2026-09-26) | [Auditoría M04.1](FASES/FASE1/MF04/m04-1-auditoria.md) |
...
- **`H-E1-19`** — El contrato K02 (`src/modules/integrations/contract/oauth-attempt.ts`) usa dos campos de `oauth_attempts` que la lista de columnas del plan (II.4, paso 5) no incluye: `created_at` y `provider`. Además fija la vida máxima del intento en 10 minutos (`OAUTH_ATTEMPT_MAX_TTL_MS`). La spec (CA-02) solo pide un vencimiento "explícito y corto", así que ese valor era propio. *Impacto:* si la migración `0012` no agrega esas columnas, una fila leída de la base no valida contra K02. *Evidencia:* plan II.4, paso 5, y `oauth-attempt.ts`. *Estado:* el usuario ratificó el tope de 10 minutos al aprobar `G-K02-K04` el 2026-09-26; ya no es un valor pendiente de decisión. *Asignación:* M06.1a agrega `created_at` y `provider` a `public.oauth_attempts`, y decide si la base hace cumplir el TTL ratificado con un `check`. *Dependencias relacionadas:* `token_type` tiene como único valor `system_user` y la credencial exige `ads_read`, las dos cosas derivadas de DEC-02 y CA-32. Si H-02 o DEC-02 cambian, ese enum cambia.
...
## Detalle de los hallazgos registrados al preparar M06.1a-M06.2a
